...
export default function TrackOrder() {
  ...
  function renderOrderCard(order, idx) {
    ...
    return (
      ...
      >
        {/* Card Top bar */}
        <div className="p-5 border-b border-white/5 bg-[#0a0f24]/50">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0">
              <div className="h-12 w-12 rounded-xl bg-blue-600/10 border border-blue-500/10 flex items-center justify-center shrink-0">
          ...
          <div className="mt-5 h-2 rounded-full bg-[#070b19] overflow-hidden border border-white/5">
            <motion.div
          ...
          <div className="flex justify-between mt-2.5 text-[9px] font-bold text-slate-500 uppercase tracking-widest">
            <span>Placed</span>
            <span>Confirmed</span>
                    ...
                    return (
                      <div key={step.key} className="flex gap-4">
                        <div className="flex flex-col items-center">
                          ...
                          >
                            {isCompleted ? <StepIcon className="h-4 w-4" /> : <Circle className="h-2.5 w-2.5 fill-current" />}
                          </motion.div>
                          {!isLast && <div className={`w-0.5 h-8 transition-colors ${isCompleted && i < currentIdx ? "bg-blue-500/20" : "bg-white/5"}`} />}
                        </div>
                        <div className={`pb-4 ${isCurrent ? "" : isCompleted ? "opacity-75" : "opacity-35"}`}>
                          <p className={`text-sm font-bold ${isCurrent ? step.color : "text-white"}`}>{step.label}</p>
                          {isCurrent && (
                            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-xs text-slate-400 mt-1 max-w-md leading-relaxed">
                              {step.description}
                            </motion.p>
                          )}
                        </div>
