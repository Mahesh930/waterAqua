...
import { Star, Truck, Phone, Hash, Droplets, Navigation, MapPin, Save, Shield, CheckCircle2, Locate, User, Edit2 } from "lucide-react";
import { useToast } from "@/shared/hooks/use-toast";
import { useAuth } from "@/contexts/AuthContext";
import { useGetMySupplierQuery, useUpdateSupplierMutation, useGetSupplierFeedbackQuery, useGetMeQuery, useUpdateProfileMutation } from "@/store/api";
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6 max-w-2xl mx-auto text-slate-200">
          ...
          <div className="mt-3 flex items-start justify-between">
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <Button 
                variant="outline" 
                size="sm" 
                className="rounded-xl border-white/10 hover:bg-white/5 text-slate-300 hover:text-white mt-1" 
      ...
      <motion.div variants={item} className="bg-[#0e142e]/60 border border-white/5 rounded-2xl p-5 flex items-center justify-between shadow-md">
        ...
        <div className="flex items-center gap-3">
          ...
          <Switch checked={form.available} onCheckedChange={v => setForm({ ...form, available: v })} disabled={!editing} />
        </div>
      </motion.div>
      ...
      <motion.div variants={item} className="bg-[#0e142e]/60 border border-white/5 rounded-2xl p-5 space-y-4 shadow-md">
        <h3 className="font-bold text-white text-base flex items-center gap-2 border-b border-white/5 pb-2">
          <User className="h-4 w-4 text-blue-400" /> Personal Profile Settings
        </h3>
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <Label className="text-xs text-slate-400">Full Name</Label>
            <Input 
              className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white" 
          ...
          <div className="sm:col-span-2">
            ...
            <Input 
              className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white opacity-60 cursor-not-allowed" 
      ...
      <motion.div variants={item} className="bg-[#0e142e]/60 border border-white/5 rounded-2xl p-5 space-y-4 shadow-md">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <Input className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white" value={form.business_name} onChange={e => setForm({ ...form, business_name: e.target.value })} disabled={!editing} />
          </div>
          <div>
            ...
            <Input className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white" placeholder="6-digit pincode" maxLength={6}
              value={form.pincode} onChange={e => setForm({ ...form, pincode: e.target.value.replace(/\D/g, "").slice(0, 6) })} disabled={!editing} />
          ...
          <div className="sm:col-span-2">
            ...
            <Input className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white" value={form.area} onChange={e => setForm(prev => ({ ...prev, area: e.target.value }))} disabled={!editing} />
          </div>
          <div className="sm:col-span-2">
            <Label className="text-xs text-slate-400">Full Business Address (Manual)</Label>
            <Input className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white" placeholder="e.g. Shop No. 4, MG Road, Bangalore" value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} disabled={!editing} />
          </div>
          <div>
            <Label className="text-xs text-slate-400">Water Type</Label>
            <Input className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white" value={form.water_type} onChange={e => setForm({ ...form, water_type: e.target.value })} disabled={!editing} />
          </div>
          <div>
            <Label className="text-xs text-slate-400">Stock (cans)</Label>
            <Input type="number" className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white" value={form.stock} onChange={e => setForm({ ...form, stock: Number(e.target.value) })} disabled={!editing} />
          </div>
          <div>
            <Label className="text-xs text-slate-400">Delivery Time</Label>
            <Input className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white" value={form.delivery_time} onChange={e => setForm({ ...form, delivery_time: e.target.value })} disabled={!editing} />
          </div>
          <div className="flex flex-col justify-end">
            <Label className="text-xs mb-1 text-slate-400">Business Geolocation</Label>
            <Button variant="outline" size="sm" onClick={handleSetLocation} className="rounded-xl gap-2 h-10 border-dashed border-blue-500/20 bg-blue-500/5 hover:bg-blue-500/10 text-white border-white/5" disabled={!editing}>
      ...
      <motion.div variants={item} className="bg-[#0e142e]/60 border border-white/5 rounded-2xl p-5 space-y-4 shadow-md">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="bg-[#090d22]/60 border border-white/5 rounded-2xl p-4 text-center">
            ...
            <Input type="number" className="mt-2 rounded-xl text-center text-lg font-bold bg-[#090d22] border-white/5 text-white" value={form.price_per_can} onChange={e => setForm({ ...form, price_per_can: Number(e.target.value) })} disabled={!editing} />
          </div>
          <div className="bg-[#090d22]/60 border border-white/5 rounded-2xl p-4 text-center">
            <Truck className="h-5 w-5 text-indigo-400 mx-auto mb-2" />
            <Label className="text-xs text-slate-400">Price per Tanker (₹)</Label>
            <Input type="number" className="mt-2 rounded-xl text-center text-lg font-bold bg-[#090d22] border-white/5 text-white" value={form.price_per_tanker} onChange={e => setForm({ ...form, price_per_tanker: Number(e.target.value) })} disabled={!editing} />
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={item} className="bg-[#0e142e]/60 border border-white/5 rounded-2xl p-5 space-y-4 shadow-md">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <Input type="number" className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white" value={form.tanker_capacity} onChange={e => setForm({ ...form, tanker_capacity: Number(e.target.value) })} disabled={!editing} />
          </div>
          <div>
            <Label className="text-xs text-slate-400 flex items-center gap-1">
              <Phone className="h-3 w-3 text-blue-400" /> Driver Phone
            </Label>
            <Input className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white" placeholder="+91 98765 43210" value={form.driver_phone} onChange={e => setForm({ ...form, driver_phone: e.target.value })} disabled={!editing} />
          </div>
          <div className="sm:col-span-2">
            <Label className="text-xs text-slate-400 flex items-center gap-1">
              <Hash className="h-3 w-3 text-blue-400" /> Vehicle Number
            </Label>
            <Input className="mt-1 rounded-xl bg-[#090d22] border-white/5 text-white" placeholder="MH 12 AB 1234" value={form.vehicle_number} onChange={e => setForm({ ...form, vehicle_number: e.target.value })} disabled={!editing} />
          </div>
        </div>
      </motion.div>
      ...
      {supplier && (
        <motion.div variants={item} className="bg-[#0e142e]/60 border border-white/5 rounded-2xl p-5 shadow-md">
          <ServiceAreaManager disabled={!editing} />
        </motion.div>
      )}
