...
import { Search, MapPin, Navigation, ShoppingCart, Plus, Filter, Droplets, Star, Clock, Locate } from "lucide-react";
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={item}>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          {hasPincode ? `${filtered.length} products near ${pincodeData.area}` : "Enter your pincode to see available products"}
        </p>
      </motion.div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="font-heading font-semibold text-lg">No products in your area</p>
          <p className="text-sm text-muted-foreground mt-1">Try a different pincode or category</p>
        </div>
      ) : hasPincode && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {filtered.map(p => {
            const eta = estimateDeliveryTime(p.suppliers?.pincode, pincodeInput);
            return (
              <motion.div key={p.id} variants={item}
                whileHover={{ y: -4, transition: { duration: 0.2 } }}
                className="glass-card rounded-2xl overflow-hidden group">
                {/* Product image placeholder */}
                <div className="relative h-28 bg-gradient-to-br from-primary/5 via-accent/5 to-transparent flex items-center justify-center">
                  <span className="text-4xl">{
                    p.category === "bottle" ? "🍶" : p.category === "tanker" ? "🚛" : p.category === "jar" ? "🫙" : "🪣"
                  }</span>
                ...
                <div className="p-3 space-y-2">
                  ...
                  <div className="flex items-center gap-1 text-[11px] text-muted-foreground">
                    ...
                    <span className="font-medium">{Number(p.suppliers?.rating || 0).toFixed(1)}</span>
                    <span>· {p.suppliers?.business_name}</span>
                  </div>
                  <div className="flex items-center gap-1 text-[11px] text-muted-foreground">
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    <Button size="sm" className="rounded-xl h-8 px-3 text-xs gap-1"
